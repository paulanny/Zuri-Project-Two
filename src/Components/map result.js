...
function App() {
  ...
  useEffect(() => {
    ...
    // Set the fetched movies into setSearchResults
    const fetchData = async () => {
      // Fetch your initial movie list and set it to setSearchResults
      // For example:
      const apiKey = '8b6e37cfbe4ef31a23e07a928d019469';
      const page = 1; // Replace with the desired page number
      const apiUrl = `https://api.themoviedb.org/3/search/movie?api_key=${apiKey}&include_adult=false&language=en-US&page=1`;
      ...
      {loading ? (
        ...
        <div>
          {showSearchResults ? (
            ...
            <CardLists movies={searchResults} />
          )}
        </div>
      )}
      <Footer />
...
// incase router goes wrong: fallack code
